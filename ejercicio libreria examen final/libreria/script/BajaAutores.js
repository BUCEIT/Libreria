//Pongo a escuchar la carga de la pagina
            ...
            function traer(){
                ...
                obj.onreadystatechange=function(){
                    if((obj.status==200)&&(obj.readyState==4)){
                        respuesta=JSON.parse(obj.responseText); //Lo que me trae ajax
                        console.log(respuesta);
                        var i;
                        //Creo los option y se los añado al select
                        for(i=0;i<respuesta.length;i++){
                            option=document.createElement("option");
                            option.setAttribute("value",respuesta[i]["codautor"]);
                ...
                obj.open("get","../includes/traer.php?pag=1");//(metodo,url,si es asincrona,usuario,contraseña) Ahora el estado vale 1, 
            ...
            function borra(){
                ...
                //Tambien le puedo pasar parametros y variables a la vez que establezco la conexion concatenando en la url
                obj2.open("get","../includes/delete.php?pag=3&dato="+dato);//(metodo,url,si es asincrona,usuario,contraseña) Ahora el estado vale 1, 
                //Enviamos la peticion al servidor
